...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    const stringWithoutFirstSymbol = `${str.slice(0, i)}${str.slice(i + 1)}`;
    if (!stringWithoutFirstSymbol.includes(str[i])) {
      return str[i];
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  ...
  const control = card.shift();
  let result = 0;
  for (let i = 0; i < card.length; i += 2) {
    card[i] = Number(card[i]) * 2;
  }
  for (let i = 0; i < card.length; i += 1) {
    if (card[i] > 9) {
      card[i] -= 9;
    }
    result += Number(card[i]);
  }
...
function isBracketsBalanced(str) {
  ...
  const stack = [];
  const bracketsPair = Object.fromEntries(bracketsConfig);

  for (let i = 0; i < strArr.length; i += 1) {
    const symbol = stack[stack.length - 1];
    if (bracketsPair[symbol] === strArr[i]) {
      stack.pop();
    } else if (bracketsPair[strArr[i]]) {
      stack.push(strArr[i]);
    } else {
      return false;
    }
...
function getCommonDirectoryPath(pathes) {
  const firstPath = pathes.map((path) => path.split('/'))[0];
  const splittes = pathes.map((path) => path.split('/'));
  for (let i = 0; i < firstPath.length; i += 1) {
    for (let j = 1; j < pathes.length; j += 1) {
      if (splittes[j][i] !== firstPath[i]) {
        if (i === 0) {
          return '';
        }
